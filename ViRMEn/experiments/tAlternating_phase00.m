function code = tAlternating_phase00
...
vr.session = struct(...
    ...
    'mazeName','tAlternating_phase00',... % change this to get the actual maze name from vr
...
if vr.mazeEnded
    ...
    if rewardsPerMinute > vr.session.targetRPM
        ...
        vr.trial(vr.tN+1).itiDuration = min(vr.trial(vr.tN).itiDuration + vr.session.incrementITI, vr.session.maxITI);
        %vr.trial(vr.tN+1).armLength = min((vr.trial(vr.tN).armLength + abs(rewardsPerMinute - vr.session.targetRPM)), vr.session.maxArmLength);
    else
        % make the maze easier by decreasing the ITI
        vr.trial(vr.tN+1).itiDuration = max(vr.trial(vr.tN).itiDuration - vr.session.incrementITI, vr.session.minITI);
        %vr.trial(vr.tN+1).armLength = max((vr.trial(vr.tN).armLength - abs(abs(rewardsPerMinute - vr.session.targetRPM))), vr.session.minArmLength);
    end
    ...
    if percentageCorrectLast20 > 0.5
        probTwoTowers = (percentageCorrectLast20-0.5)*2;
        vr.trial(vr.tN+1).twoTowerTrial = rand(1)<probTwoTowers;
    end
