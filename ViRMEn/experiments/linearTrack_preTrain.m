...
vr.worlds{1}.surface.colors(1,:)=0;
vr.worlds{2}.surface.colors(1,:)=0;
vr.worlds{3}.surface.colors(1,:)=0;
...
elseif (now-vr.trial(vr.tN).start)*24*60*60 > vr.session.trialMaxDuration || vr.position(2)<(vr.trial(vr.tN).startPosition(2)-15)
% TRIAL TIMED OUT 
    vr.mazeEnded = 1;
    vr.isCorrect = 0;
...
elseif vr.position(2)>=785
    ...
    vr = giveReward(vr,4);
    vr.trial(vr.tN).totalReward = vr.trial(vr.tN).totalReward+4;
...
if vr.trialEnded
    ...
    vr.rpm = sum([vr.trial(lastNTrials).totalReward])./sum([vr.trial(lastNTrials).duration]);
    vr.rpm = vr.rpm*60/4;
    scale = vr.rpm-vr.session.targetRPM;
    startY = vr.trial(vr.tN-1).startPosition(2)-scale;
    startY(startY<10) = 10;
    startY(startY>783) = 783;

%     if vr.rpm>vr.session.targetRPM
%         startY = max(vr.trial(vr.tN-1).startPosition(2)-scale,10);
%     else
