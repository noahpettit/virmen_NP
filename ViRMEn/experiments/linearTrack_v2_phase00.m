...
vr.session = struct(...
    ...
    'mazeName','linearTrack_v2_phase00',... % change this to get the actual maze name from vr
...
vr.iter = struct(...
    ...
    'startTimeInTrial',toc(vr.trial(1).tic),... % time in trial (from trial start)
    'startTimeInSession',toc(vr.session.tic), ... % time in session (from session start)
    'pitchRollYaw',[]...
    );
...
vr.trial(vr.tN).correctTarget = [vr.trial(vr.tN).targetDir*vr.trial(vr.tN).tDist, 20];
vr.trial(vr.tN).incorrectTarget = [-1*vr.trial(vr.tN).targetDir*vr.trial(vr.tN).tDist, 20];
...
if vr.trialEnded
    ...
    rewardsPerMinute = rewardCounter/(min(secCounter/60,1));
    ...
    vr.trial(vr.tN).stemLength = max(vr.session.minStemLength, min(vr.session.maxStemLength, vr.trial(vr.tN-1).stemLength+(rewardsPerMinute-vr.session.targetRPM)));
    
    % update conditions based on rpm?
    if vr.trial(vr.tN).stemLength >= vr.session.maxStemLength-10
        vr.trial(vr.tN).tDist = min(15, vr.trial(vr.tN-1).tDist+1);
    else
        vr.trial(vr.tN).tDist = max(0, vr.trial(vr.tN-1).tDist-1);
    end
    ...
    vr.exper.variables.leftTowerX = num2str(vr.trial(vr.tN).tDist+(~vr.trial(vr.tN).leftTowerVisible*20));
    vr.exper.variables.rightTowerX = num2str(-vr.trial(vr.tN).tDist-(~vr.trial(vr.tN).rightTowerVisible*20));
    ...
    vr.text(7).string = upper(['TDIST: ', num2str(abs(vr.trial(vr.tN).tDist))]);
...
if ~(vr.isFrozen || vr.isBlackout)
    if vr.drawText
        ...
        vr.text(10).string = upper(['FRZN: ', num2str(abs(vr.trial(vr.tN).frozenDuration))]);
        vr.text(12).string = upper(['BO: ', num2str(abs(vr.trial(vr.tN).blackoutDuration))]);
        vr.text(13).string = upper(['MAXSL: ', num2str(max([vr.trial(:).stemLength]))]);
        vr.text(14).string = upper(['FR: ' num2str(round(length(vr.iter)/toc(vr.iter(1).tic)))]);
        vr.text(15).string = upper(['NUM15: ' num2str(sum([vr.trial(:).tDist]==15))]);
