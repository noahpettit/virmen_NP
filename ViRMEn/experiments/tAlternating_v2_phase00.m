...
if vr.keyPressed==82 % R key to give reward manually
    ...
    vr.iter(vr.iNT).rewardN = vr.iter(vr.iNT).rewardN+vr.isReward;
end
...
if vr.trialEnded
    ...
    if sum([vr.trial(:).twoTowerTrial])>100 && mean([vr.trial([vr.trial(:).twoTowerTrial]).isCorrect])>0.75
    ...
    if rand(1)>(max(0, (percentageCorrectLast20 - 0.5))*2)
