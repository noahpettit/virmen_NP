function code = tAlternating_v2_phase01
...
vr.session = struct(...
    ...
    'mazeName','tAlternating_v2_phase01',... % change this to get the actual maze name from vr
...
elseif vr.fun.euclideanDist(vr.position(1:2),vr.trial(vr.tN).correctTarget)<vr.trial(vr.tN).correctRadius
    ...
    % compute amount of reward to give 
...
if vr.trialEnded
    ...
    if sum([vr.trial(:).twoTowerTrial])>50 && mean([vr.trial([vr.trial(:).twoTowerTrial]==1).isCorrect])>0.70
    ...
    if percentageCorrectLast20 > 0.70
